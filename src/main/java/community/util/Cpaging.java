...
public class Cpaging {
	...
	//총 게시글 수를 입력하는 생성자
		...
		public Cpaging(int totalCount, int curPage) {
			setTotalCount(totalCount);
			setCurPage(curPage);
		...
		public Cpaging(int totalCount, int curPage, int listCount) {
			setTotalCount(totalCount);
			setCurPage(curPage);
		...
		public Cpaging(int totalCount, int curPage, int listCount, int pageCount) {
			setTotalCount(totalCount);
			setCurPage(curPage);
			setListCount(listCount);
		...
		private void makePaging() {
			...
			if(pageCount == 0)	setPageCount(6);//화면에 보여질 페이지 수 기본설정
